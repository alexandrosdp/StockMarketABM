...
def model(params_chunk):
    results = []
    num_runs = 100  # Number of runs to calculate the crash probability
    for params in params_chunk:
        ...
        # Run the experiment multiple times and calculate the probability of a crash
        crash_count, _ = exp.multiple_runs_crash(num_runs)
        crash_probability = crash_count / num_runs
        results.append(crash_probability)
    return results
...
if __name__ == '__main__':
    ...
    axes[1].set_title(
        'PAWN Sensitivity Analysis (Median)- probability of crashes')
    ...
    plt.tight_layout()
    plt.savefig('pawn_sensitivity_analysis- probability of crashes.svg')
    plt.show()
